// scripts/withdrawFees.js
const hre = require("hardhat");
require("dotenv").config();

// 确保这是你部署在 imua 链上的 LockTokens 合约的正确地址
const LOCK_TOKENS_ADDRESS = "0x64FE7cac156E74Fd3437A26a2FcCEF0b2fE1B60B"; 
async function main() {
  ...
  if (!LOCK_TOKENS_ADDRESS || LOCK_TOKENS_ADDRESS === "YOUR_LOCK_TOKENS_ADDRESS_GOES_HERE") {
    console.error("错误: 请在 scripts/withdrawFees.js 中更新 LOCK_TOKENS_ADDRESS");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`当前 LockTokens 合约中累积的费用: ${hre.ethers.formatEther(currentFees)} imua`);
    ...
    console.log(`正在从 LockTokens 合约提取 ${hre.ethers.formatEther(currentFees)} imua 费用...`);
    ...
    console.log(`提取后 LockTokens 合约中剩余的费用: ${hre.ethers.formatEther(remainingFees)} imua`);

  } catch (error) {
    console.error("提取费用失败:", error.message);
    console.error("请确保运行此脚本的账户是 LockTokens 合约的 owner，并且有足够的 imua 支付 Gas 费。");
    process.exit(1);
  }
}
